...
for wbin in sdict["INFO"]["BINS"]:
    ...
    for night in range(nights):
        ...
        ax1.errorbar(phasei,rvi,sigrvi,fmt="b.", ecolor="green", capsize=2.,ms=4,capthick=0.5,elinewidth=0.7,zorder=10)
        ...
        ax1.errorbar(phasei,rvi_kp,1000.*uncorr_err,fmt="b.", ecolor="green", capsize=2.,ms=4,capthick=0.5,elinewidth=0.7,zorder=10)
        ###
        ...
        ax1.errorbar(phasei,rvi_err,1000.*uncorr_err,fmt="b.", ecolor="green",capsize=2.,ms=4,capthick=0.5,elinewidth=0.7,zorder=10)
        if gps==True:
...
# Plots the transit ingress and egress
    ...
    ax1.errorbar(joint_phase,joint_rv_res,joint_sigrv,fmt="b.", ecolor="green", capsize=2.,ms=4,capthick=0.5,elinewidth=0.7,zorder=10)
    ax1.set_ylabel("RV [m/s]")
    ax1.set_xlabel("Phase")
    ...
    ax1.errorbar(phasei,rvi,np.array(joint_sigrv),fmt="b.", ecolor="green", capsize=2.,ms=4,capthick=0.5,elinewidth=0.7,zorder=10)
    ###
    fmodel,_,sigw,_,_=dpcy(fitmodel(phasen,nguessdict[night]))
    ...
    ax1.plot(phasen,1000*(m-rvfitk),color="r",linewidth=0.5)
    mdl=1000*(m-rvfitk)
    Amp+=[[min(mdl), max(mdl), std_finalerror]]
    ####
# Plots the transit ingress and egress
    ax1.axvspan(-tr_dur/2.-tr_dict["dT0"], tr_dur/2.-tr_dict["dT0"], alpha=0.3, color='orange')
    ax1.axvspan(-tr_ingress_egress/2.-tr_dict["dT0"], tr_ingress_egress/2.-tr_dict["dT0"], alpha=0.4, color='orange')
    ...
    if gps==True:
        ...
        ax1.errorbar(joint_phase,joint_rv_res2,joint_sigrv,fmt="b.", ecolor="green", capsize=2.,ms=4,capthick=0.5,elinewidth=0.7,zorder=10)
        ax1.set_ylabel("RV [m/s]")
        ax1.set_xlabel("Phase")
...
f0= open("amperr.txt", "w+")
f0.write("Max_RV"+";"+"Min_RV"+";"+"RV_ERR")
for k in Amp:
    f0.write(str(k[0])+";"+str(k[1])+";"+str(k[2])+"\n")
